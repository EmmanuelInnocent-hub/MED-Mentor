...
export default function Leaderboard() {
  ...
  const filteredRanks = currentRanks.filter(
    (r) =>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0a0e14] text-[#e8edf5] font-sans h-full overflow-hidden">
      <div className="flex flex-1 overflow-hidden h-full">
        {/* Sidebar - Personal Ranks */}
        <aside className="hidden xl:flex w-72 flex-col border-r border-[#1e2a3a] bg-[#111620] shrink-0 overflow-hidden">

          <div className="p-6 border-b border-[#1e2a3a]">
            <h3 className="text-[10px] font-mono uppercase tracking-widest text-[#5a7090]">
              Global Filter
            </h3>
          ...
          <div className="p-6 m-4 bg-gradient-to-br from-blue-600/20 to-blue-600/5 rounded-2xl border border-blue-500/20 transition-all hover:border-blue-500/40">
            <div className="text-[10px] font-mono uppercase tracking-widest text-blue-400 mb-6">
              Your position
            </div>
            <div className="text-5xl font-serif text-white mb-2 leading-none italic">
              {filter === "global" ? "N/A" : "N/A"}
            </div>
            <div className="text-sm font-bold text-white mb-1">
              Student Practitioner
            </div>
            <div className="text-[11px] font-mono text-[#5a7090]">
              0% avg · 0 cases · 0🔥
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
            ...
            {[
              ...
              <div
                key={spec.s}
                className="flex items-center justify-between py-2 border-b border-white/5 group cursor-pointer hover:border-white/10 transition-all"
              >
                <span className="text-xs text-white/50 group-hover:text-white transition-colors">
                  {spec.s}
                </span>
                <span className={`text-xs font-mono font-bold ${spec.c}`}>
                  {spec.r}
                </span>
              </div>
            ))}
        ...
        {/* Main Content Area */}
        <section className="flex-1 flex flex-col min-w-0 bg-[#0a0e14] overflow-hidden">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-lg font-serif italic text-white tracking-tight">
                  Leaderboard{" "}
                  <span className="text-sm font-sans font-light not-italic opacity-30 ml-2">
                    / {filter.toUpperCase()}
                  </span>
                </h1>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 lg:p-12 space-y-12 custom-scrollbar">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {top3Global
                  .sort((a, b) => (a.pos === 1 ? 0 : a.pos === 2 ? -1 : 1))
                  .map((hero) => (
                    <div
                      key={hero.id}
                      className={`flex flex-col items-center gap-4 ${hero.pos === 1 ? "order-2" : hero.pos === 2 ? "order-1 pb-4" : "order-3 pb-4"}`}
                    >
                      <div
                        className={`w-14 h-14 md:w-24 md:h-24 rounded-full flex items-center justify-center border-2 bg-[#111620] relative shadow-2xl ${
                          hero.color === "gold"
                      ...
                      >
                        ...
                        <div className="absolute -bottom-2 px-2 py-0.5 rounded bg-black/80 text-[8px] font-mono border border-white/10 uppercase tracking-widest">
                          {hero.color}
                        </div>
                      </div>
                      <div className="text-center">
                        ...
                        <div className="text-[10px] font-mono text-[#5a7090] mt-0.5">
                          {hero.school}
                        </div>
                      </div>
                      <div
                        className={`text-sm md:text-xl font-serif font-black ${hero.color === "gold" ? "text-amber-500" : hero.color === "silver" ? "text-slate-400" : "text-amber-900"}`}
                      >
                        {hero.score}%
                      </div>
                      <motion.div
                        ...
                        animate={{
                          height:
                            hero.pos === 1 ? 128 : hero.pos === 2 ? 96 : 64,
                        }}
                        className={`w-full rounded-t-2xl bg-gradient-to-t border-t ${
                          hero.pos === 1
                            ? "from-amber-500/5 to-amber-500/20 border-amber-500/20"
                            : hero.pos === 2
                              ? "from-slate-400/5 to-slate-400/15 border-slate-400/20"
                              : "from-amber-900/5 to-amber-900/15 border-amber-900/20"
                        }`}
                      />
                    </div>
                  ))}
        ...
        <aside className="hidden 2xl:flex w-[400px] flex-col bg-[#111620] border-l border-[#1e2a3a] shrink-0 overflow-hidden">


          <div className="p-10 space-y-12 no-scrollbar overflow-y-auto">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {[
                  {
                    t: "Neuro Sprint",
                    d: "Complete 3 more Neuro cases with 70%+",
                    p: 40,
                    c: "bg-purple-500",
                  },
                  {
                    t: "Pharma Master",
                    d: "Score 80%+ on any 3 drug quizzes",
                    p: 33,
                    c: "bg-amber-500",
                  },
                  {
                    t: "Grit Award",
                    d: "Study for 7 consecutive days",
                    p: 85,
                    c: "bg-teal-500",
                  },
                ].map((ch) => (
                  <div
                    key={ch.t}
                    className="bg-[#161d2a] border border-[#243044] rounded-3xl p-6 group hover:border-blue-500/20 transition-all"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="text-xs font-bold text-white group-hover:text-amber-500 transition-colors uppercase font-mono tracking-tighter">
                        {ch.t}
                      </div>
                      <div className="text-[9px] font-mono text-green-500 flex items-center gap-1 font-bold">
                        +500 PX
                      </div>
                    </div>
                    <p className="text-[11px] text-[#5a7090] leading-relaxed mb-5 font-medium">
                      {ch.d}
                    </p>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div
                    key={i}
                    className="aspect-square bg-[#161d2a] border border-[#243044] rounded-3xl flex flex-col items-center justify-center gap-1 grayscale hover:grayscale-0 transition-all cursor-help hover:border-amber-500/30 group"
                  >
                    <span className="text-2xl group-hover:scale-110 transition-transform">
                      {badge.e}
                    </span>
                    <span className="text-[7px] font-mono text-[#5a7090] font-bold uppercase">
                      {badge.l}
                    </span>
                  </div>
                ))}
